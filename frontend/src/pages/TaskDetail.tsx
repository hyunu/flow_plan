  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <Link to={`/projects/${task.project_id}`} className="inline-flex items-center gap-1.5 text-[13px] text-slate-400 hover:text-ink-700">
            <IconArrowLeft size={14} /> 프로젝트 현황판
          </Link>
      ...
      <div className="card p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-ink-900">진척률</h3>
        </div>
        <div className="grid sm:grid-cols-4 gap-3">
          {progressValues.map(({ label, v, sub }) => (
            <div key={label} className="rounded-xl bg-surface-50 px-4 py-3 ring-1 ring-slate-100">
              <div className="text-[11px] font-semibold text-slate-400 uppercase">{label}</div>
              <div className="text-2xl font-bold text-ink-900 mt-0.5">{Math.round(v)}%</div>
              <div className="text-[11px] text-slate-400">{sub}</div>
            </div>
          ))}
        </div>
        <ProgressBar value={task.effective_progress} className="mt-4" />
          ...
          {task.assignments.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {task.assignments.map((a) => (
                <span key={a.id} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-50 ring-1 ring-slate-200 text-sm">
