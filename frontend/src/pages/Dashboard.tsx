...
export function Dashboard() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-5 animate-fade-in">
      ...
      {/* AI 요약 */}
      {data.ai_summary && (
        <div className="card p-6 bg-gradient-to-br from-brand-50/60 to-white">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="card p-6">
          <PanelHeader title="Issue" icon={<IconAlert size={15} />} />
          {data.issues.length === 0 ? (
            <EmptyState>Issue가 없습니다</EmptyState>
          ) : (
            <div className="space-y-2">
              {data.issues.map((i) => (
                <div key={i.id} className="px-3 py-2.5 rounded-xl bg-amber-50/60">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[13px] font-medium text-ink-700 truncate">{i.title}</span>
                    <StatusBadge status={i.status} />
                  </div>
                  {i.cause && <div className="text-xs text-slate-500 mt-1">원인: {i.cause}</div>}
                  {i.resolve_plan_date && (
        ...
        <div className="card p-6">
          ...
          <div className="space-y-3">
            {data.user_workload.map((u) => (
              <div key={u.user_id} className="flex items-center gap-4 px-3 py-2.5 rounded-xl bg-surface-50">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] font-medium text-ink-700">{u.name}</div>
                  <ProgressBar
          ...
          {data.recent_changes.length === 0 ? (
            ...
            <div className="space-y-2">
              {data.recent_changes.map((c, i) => (
                <div key={i} className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg hover:bg-surface-50">
                  <div className="min-w-0">
                    <div className="text-[13px] text-ink-700">Task #{c.task_id}</div>
